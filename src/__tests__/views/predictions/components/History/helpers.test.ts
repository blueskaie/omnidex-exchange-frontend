...
import { formatTlos, formatUsd, getMultiplier, getPayout } from 'views/Predictions/components/History/helpers'
...
describe('formatTlos', () => {
  it.each([
    ...
    [689.889, '689.889'],
    [10.8829, '10.883'],
  ])('format %i TLOS correctly with 3 decimals', (value, expected) => {
    expect(formatTlos(value)).toEqual(expected)
  })

  it('returns 0 if TLOS is undefined', () => {
    expect(formatTlos(undefined)).toEqual('0')
  })
})
