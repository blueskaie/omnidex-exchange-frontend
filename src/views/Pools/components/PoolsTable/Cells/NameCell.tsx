...
import { Text, useMatchBreakpoints, Link } from 'pancakeswap-uikit'
...
const NameCell: React.FC<NameCellProps> = ({ pool }) => {
  ...
  if (isAutoVault) {
    ...
    title = t('Stake CHARM')
    if (sousId !==0){
      subtitle = `${t('Stake')} CHARM ${t('Earn').toLocaleLowerCase()} CHARM`
    }
    else {
      subtitle = t('')
    }
    ...
    <StyledCell role="cell">
      ...
      <CellContent>
        ...
        <StyledLink external href="/karmapool">
          <Text fontSize="12px" color="textSubtle"> {getFinishingHeading()} </Text>
        </StyledLink>
      </CellContent>
    </StyledCell>
  )
