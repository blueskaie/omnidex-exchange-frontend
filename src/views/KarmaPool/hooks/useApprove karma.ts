...
export const useVaultApprove = (setLastUpdated: () => void) => {
  ...
  const { account, library } = useActiveWeb3React()
  const { anchorSession } = useContext(AnchorContext)
  const cakeVaultContract = useDojoContract()
  const cakeContract = useKarmaWithAccount()
...
export const useCheckVaultApprovalStatus = () => {
  ...
  const karmaContract = useKarmaWithAccount()  
  const dojoContract = useDojoContractWithAccount()  
  const { lastUpdated, setLastUpdated } = useLastUpdated()
  useEffect(() => {
    const checkApprovalStatus = async () => {
      try {
        const response = await karmaContract.allowance(account, dojoContract.address)   
    ...
    checkApprovalStatus()
  }, [account, karmaContract, dojoContract, lastUpdated])  
