      ...
      toastSuccess(
        ...
        <Box>
          ...
          {receipt.transactionHash && (
            <LinkExternal href={getTelosExplorerLink(receipt.transactionHash, 'transaction')}>
      ...
      <ModalBody p="24px">
        ...
        <Flex alignItems="start" justifyContent="space-between" mb="8px">
          ...
          <Box style={{ textAlign: 'right' }}>
            <Text>{`${betAmount} TLOS`}</Text>
            <Text fontSize="12px" color="textSubtle">
              {`~$${tlosBusdPrice.times(betAmountAsFloat).toFormat(2)}`}
            </Text>
          </Box>
        </Flex>
        <Flex alignItems="start" justifyContent="space-between" mb="24px">
          ...
          <Box style={{ textAlign: 'right' }}>
            <Text>{`${payout} TLOS`}</Text>
            <Text fontSize="12px" color="textSubtle">
              {`~$${tlosBusdPrice.times(payoutAsFloat).toFormat(2)}`}
            </Text>
          </Box>
        </Flex>
        <Button
