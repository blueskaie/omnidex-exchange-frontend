...
import { formatTlos } from '../helpers'
...
const SummaryRow: React.FC<SummaryRowProps> = ({ type, summary, tlosBusdPrice }) => {
    ...
    <>
      ...
      <Flex>
        ...
        <Flex flex="3" flexDirection="column">
          <Text bold fontSize="20px" color={color}>
            {`${summaryTypeSigns[type]}${formatTlos(displayAmount)} TLOS`}
          </Text>
          <Text fontSize="12px" color="textSubtle">
            {`~$${formatTlos(tlosBusdPrice.times(displayAmount).toNumber())}`}
          </Text>
        </Flex>
      </Flex>
    </>
