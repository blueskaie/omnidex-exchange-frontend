...
import { formatTlos, getNetPayout } from './helpers'
      ...
      <StyledBetResult>
        ...
        {bet.claimed && (
          <Flex justifyContent="center">
            <LinkExternal href={getTelosExplorerLink(bet.claimedHash, 'transaction')} mb="16px">
        ...
        <Flex alignItems="center" justifyContent="space-between" mb="16px">
          ...
          <Text>{`${formatTlos(bet.amount)} TLOS`}</Text>
        </Flex>
        <Flex alignItems="start" justifyContent="space-between">
          <Text bold>{isWinner ? t('Your winnings') : t('Your Result')}:</Text>
          <Box style={{ textAlign: 'right' }}>
            <Text bold color={getResultColor()}>{`${isWinner ? '+' : '-'}${formatTlos(payout)} TLOS`}</Text>
            <Text fontSize="12px" color="textSubtle">
              {`~$${formatTlos(tlosBusdPrice.times(payout).toNumber())}`}
            </Text>
          </Box>
        </Flex>
        {isWinner && (
          <>
            ...
            <Flex alignItems="start" justifyContent="space-between">
              ...
              <Flex justifyContent="end">
                <Text fontSize="14px" color="textSubtle">{`${formatTlos(returned)} TLOS`}</Text>
                <span ref={targetRef}>
