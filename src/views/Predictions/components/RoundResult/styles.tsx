...
import { formatUsdv2, formatTlosv2 } from '../../helpers'
import { formatTlos, formatUsd } from '../History/helpers'
...
const getPrizePoolAmount = (totalAmount: PrizePoolRowProps['totalAmount']) => {
  ...
  return formatTlosv2(totalAmount)
}
...
export const PrizePoolRow: React.FC<PrizePoolRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      ...
      <Text bold>{`${getPrizePoolAmount(totalAmount)} TLOS`}</Text>
    </Row>
  )
}
  ...
  return (
    <Row height="18px" {...props}>
      ...
      <Flex alignItems="center">
        ...
        <Text mx="4px">|</Text>
        <Text fontSize="12px" lineHeight="18px">{`${formatTlos(amount)} TLOS`}</Text>
      </Flex>
    </Row>
  )
}
...
export const PrizePoolHistoryRow: React.FC<PrizePoolHistoryRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text bold>{t('Prize Pool')}:</Text>
      <Text bold>{`${getPrizePoolAmountHistory(totalAmount)} TLOS`}</Text>
    </Row>
  )
}
