...
import { useGetTlosBalance } from 'hooks/useTokenBalance'
...
const getButtonProps = (
  ...
  tlosBalance: ethers.BigNumber,
  minBetAmountBalance: ethers.BigNumber,
) => {
  const hasSufficientBalance = () => {
    if (value.gt(0)) {
      return value.lte(tlosBalance)
    }
    return tlosBalance.gt(0)
  }
...
const SetPositionCard: React.FC<SetPositionCardProps> = ({ position, togglePosition, onBack, onSuccess }) => {
  ...
  const { balance: tlosBalance } = useGetTlosBalance()
  const minBetAmount = useGetMinBetAmount()
  const { t } = useTranslation()
  const { toastError } = useToast()
  const predictionsContract = usePredictionsContract()

  // Convert tlos balance to ethers.BigNumber
  const tlosBalanceAsBn = useMemo(() => {
    return ethers.BigNumber.from(tlosBalance.toString())
  }, [tlosBalance])
  const maxBalance = useMemo(() => {
    return tlosBalanceAsBn.gt(dust) ? tlosBalanceAsBn.sub(dust) : dust
  }, [tlosBalanceAsBn])
  const balanceDisplay = formatBigNumber(tlosBalanceAsBn)
