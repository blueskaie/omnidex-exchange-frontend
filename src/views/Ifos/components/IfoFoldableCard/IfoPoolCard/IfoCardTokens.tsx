...
const CakeTlosTokenSection: React.FC<TokenSectionProps> = (props) => {
  ...
  const renderTokenSection = () => {
    ...
    if (account && !hasProfile) {
      if (publicIfoData.status === 'finished') {
        return <Text textAlign="center">{t('Activate OmniDex Profile to take part in next IFO‘s!')}</Text>
      }
      return (
        <Text textAlign="center">{t('You need an active OmniDex Profile to take part in an IFO!')}</Text>
      )
    }
