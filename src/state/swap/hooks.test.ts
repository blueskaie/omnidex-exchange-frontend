...
describe('hooks', () => {
  describe('#queryParametersToSwapState', () => {
    ...
    test('output TLOS only', () => {
      expect(
        queryParametersToSwapState(
          parse('?outputCurrency=tlos&exactAmount=20.5', { parseArrays: false, ignoreQueryPrefix: true }),
        ),
      ).toEqual({
        [Field.OUTPUT]: { currencyId: 'TLOS' },
        [Field.INPUT]: { currencyId: '' },
    ...
    test('invalid recipient', () => {
      expect(
        queryParametersToSwapState(
          parse('?outputCurrency=TLOS&exactAmount=20.5&recipient=abc', { parseArrays: false, ignoreQueryPrefix: true }),
        ),
      ).toEqual({
        [Field.OUTPUT]: { currencyId: 'TLOS' },
        [Field.INPUT]: { currencyId: '' },
    ...
    test('accepts any recipient', () => {
      expect(
        queryParametersToSwapState(
          parse('?outputCurrency=TLOS&exactAmount=20.5&recipient=bob.argent.xyz', {
            parseArrays: false,
            ignoreQueryPrefix: true,
          }),
        ),
