...
const nonTlosPools = poolsConfig.filter((p) => (p.stakingToken.symbol !== 'TLOS' && p.isDojo === false)) 
...
const nonDojoPools = poolsConfig.filter((p) => p.isDojo === false) 
...
export const fetchUserBalances = async (account) => {
  ...
  const userBalance = await dojoContract.charmBalance(account)        
  const charmBalance = new BigNumber(userBalance.toString()).toJSON() 

  return { ...tokenBalances, ...tlosBalances, charmBalance }          
}
